...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y):
        ...
        self.hp, self.ammo, self.pack, self.killed_enemies, self.animation_count = 10, 30, 5, 0, 0
        ...
        self.player_stand = pygame.image.load('data\player_stand.png')
        self.player_right = [pygame.image.load(f'data\player_right_{i}.png') for i in range(1, 5)]
        self.player_left = [pygame.image.load(f'data\player_left_{i}.png') for i in range(1, 5)]
        self.player_up = [pygame.image.load(f'data\player_up_{i}.png') for i in range(1, 5)]
        self.player_down = [pygame.image.load(f'data\player_down_{i}.png') for i in range(1, 5)]
    ...
    def move(self):
        ...
        if self.left:   # Анимация перемещения влево
            screen.blit(self.player_left[self.animation_count // 4], (player.rect.x, player.rect.y))
            self.animation_count += 1
            if self.animation_count == 8:
                self.animation_count = 0
        elif self.right:     # Анимация перемещения вправо
            screen.blit(self.player_right[self.animation_count // 4], (player.rect.x, player.rect.y))
            self.animation_count += 1
            if self.animation_count == 8:
                self.animation_count = 0
        elif self.up:    # Анимация перемещения вверх
            screen.blit(self.player_up[self.animation_count // 4], (player.rect.x, player.rect.y))
            self.animation_count += 1
            if self.animation_count == 8:
                self.animation_count = 0
        elif self.down:  # Анимация перемещения вниз
            screen.blit(self.player_down[self.animation_count // 4], (player.rect.x, player.rect.y))
            self.animation_count += 1
            if self.animation_count == 8:
                self.animation_count = 0
        else:
            screen.blit(self.player_stand, (player.rect.x, player.rect.y))
...
class Spawner(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        self.past_time += 1
        if self.past_time >= self.interval and spawned_enemies != 50:
            spawned_enemies += 1
            new_enemy = Enemy(self.rect.x + 40, self.rect.y + 20, 1, rd.randint(1, 2))

            self.past_time = 0


class Enemy(pygame.sprite.Sprite):
    def __init__(self, x_enemy, y_enemy, speed, type_enemy, movement='m'):
        ...
        self.color = rd.choice(['red', 'yellow'])
        ...
        if type_enemy == 1:
            ...
            self.anim_col_vo = 3
            self.speed = speed * 6
            self.player_stand_enemy = pygame.image.load('data\_red_down_1.png')
            self.player_right_enemy = [pygame.image.load(f'data\_red_right_{i}.png') for i in range(1, 4)]
            self.player_left_enemy = [pygame.image.load(f'data\_red_left_{i}.png') for i in range(1, 4)]
            self.player_up_enemy = [pygame.image.load(f'data\_red_up_{i}.png') for i in range(1, 3)]
            self.player_down_enemy = [pygame.image.load(f'data\_red_down_{i}.png') for i in range(1, 3)]
    ...
    def update(self):
        ...
        if abs(self.rect.x - x) <= 350 and abs(self.rect.y - y) <= 350 and self.type_enemy == 1:
            self.animation = [False, False, False, False, True]
        elif abs(self.rect.x - x) <= 0 and abs(self.rect.y - y) <= 0 and self.type_enemy == 2:
            self.animation = [False, False, False, False, True]
        else:
            if abs(self.rect.x - x) >= 200 or abs(self.rect.y - y) >= 200 and self.type_enemy == 1:
                # Если игрок удаляется, враг его преследует
                self.animation[4] = False
            elif abs(self.rect.x - x) >= 0 or abs(self.rect.y - y) >= 0 and self.type_enemy == 1:
                # Если игрок удаляется, враг его преследует
                self.animation[4] = False
        if not self.animation[4] and self.move == 'm':  # Если вражеский персонаж не находится на оптимальных для стрельбы координатах
        ...
        if self.animation[0]:  # Анимация перемещения влево
            self.image = self.player_left_enemy[self.animation_count // self.anim_col_vo]
            self.animation_count = self.animation_count + 1 if self.animation_count != self.anim_col_vo else 0
        elif self.animation[1]:  # Анимация перемещения вправо
            self.image = self.player_right_enemy[self.animation_count // self.anim_col_vo]
            self.animation_count = self.animation_count + 1 if self.animation_count != self.anim_col_vo else 0
        elif self.animation[2]:  # Анимация перемещения вниз
            self.image = self.player_down_enemy[self.animation_count // self.anim_col_vo]
            self.animation_count = self.animation_count + 1 if self.animation_count != self.anim_col_vo else 0
        elif self.animation[3]:  # Анимация перемещения вверх
            self.image = self.player_up_enemy[self.animation_count // self.anim_col_vo]
            self.animation_count = self.animation_count + 1 if self.animation_count != self.anim_col_vo else 0
        else:
            self.image = self.player_stand_enemy

    def shoot(self):
        if self.animation[4] and self.type_enemy == 1:
...
def display_ui():
    for i in range(10):
        if i < player.hp:
            screen.blit(ui_images['heart'], (1254 + i * 29, 10))
        else:
            screen.blit(ui_images['empty_heart'], (1254 + i * 29, 10))
    screen.blit(ui_images['ammo_pack'], (width - 101, height - 77))
    screen.blit(ui_images['bullet'], (width - 195, height - 80))
    screen.blit(ui_images[weapon], (width - 195, height - 180))
...
ui_images = {
    ...
    'gun': load_image('gun_icon.png'),
    'knife': load_image('knife_icon.png')
}
...
while True:
    ...
    if boss != None:
        if interval < fps * 3:
            interval += 1
        if interval >= fps * 3:
            Enemy(boss.return_coords()[0], boss.return_coords()[1], 0, 1, 'm')
            interval = 0
